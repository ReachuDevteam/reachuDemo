...
const ProductDetail = ({productId, onBack}) => {
  const {productDetails, loading, error, refreshProductDetails} =
  ...
  const price =
    ...
    currentVariant.price.amount_incl_taxes
      ? {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      const response = await executeAddItemToCart(
        state.cartId,
        productDetails,
        quantity,
        variantId,
      ...
      const cartItem = response.line_items.find(
        item =>
            ...
            {productDetails.variants.map(variant => (
              <TouchableOpacity
                ...
                <Text style={styles.variantTitle}>
                  {titleCase(variant.title)}
                </Text>
                {variant.price && variant.price.amount_incl_taxes && (
                  <Text style={styles.variantPrice}>
                    {variant.price.amount_incl_taxes}{' '}
