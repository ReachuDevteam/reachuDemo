...
import { useCreateCheckout, useUpdateCheckout } from '../../../../graphql/hooks/checkout';
...
const AddressSummary = ({ title, address }) => {
    ...
    return (
        <View style={styles.addressContainer}>
            ...
            {address.address2 ? <Text style={styles.addressText}>{address.address2}</Text> : null}
            <Text style={styles.addressText}>{address.city}, {address.zip}</Text>
            <Text style={styles.addressText}>{address.country}</Text>
            <Text style={styles.addressText}>{address.phone_code} {address.phone}</Text>
        </View>
    );
};
...
export const CheckoutReview = ({
    ...
    const [acceptsPurchaseConditions, setAcceptsPurchaseConditions] = useState(true);
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            // 1. Update the cart with the country of shipment
            await executeUpdateCart(cartId, selectedCountry);
            console.log('[Review] Updated cart with country:', selectedCountry);
            ...
            for (const cartItem of cartItems) {
                ...
                if (countryId !== null) {
                    ...
                    console.log('[Review] Updated cart item shipping:', cartItem.cartItemId);
                } else {
                    console.error(
            ...
            <View style={styles.termsSection}>
                <View style={styles.checkboxContainer}>
                    <CheckBox
                        ...
                        tintColors={{ true: '#007BFF', false: termsError ? '#FF3B30' : '#757575' }}
                    />
                    <Text style={[styles.checkboxLabel, termsError && !acceptsTermsConditions && styles.errorText]}>
                ...
                <View style={styles.checkboxContainer}>
                    <CheckBox
                        ...
                        tintColors={{ true: '#007BFF', false: termsError ? '#FF3B30' : '#757575' }}
                    />
                    <Text style={[styles.checkboxLabel, termsError && !acceptsPurchaseConditions && styles.errorText]}>
